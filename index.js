...
function customObjectFromEntries(entries) {
  ...
  const prop = entries.map(([key, value]) => {
  ...
  return Object.fromEntries(prop);
}
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  if (Object.isFrozen(obj1)) {
    console.log("заморожено");
  } else {
    console.log("не заморожено");
  }
...
function keysAndValues(obj) {
  ...
  let array = [keysArray, valuesArray];
 ...
 * Функція запечатує об'єкт, а потім перевіряє, чи запечатаний об'єкт.
...
function getObjectValuesSum(obj) {
  ...
  const sum = allValues.reduce((num, value) => num + value, 0);
